...
def preprocess_text(texts: list[str], tokenizer: AutoTokenizer, max_length: int = MAX_SEQ_LENGTH) -> np.ndarray:
    ...
    """
    encoded: Callable = tokenizer(texts, padding="max_length", truncation=True, max_length=max_length, return_tensors="np")
...
def preprocess_batch(batch: Dict[str, tf.Tensor], tokenizer: Callable) -> Tuple[jnp.ndarray, jnp.ndarray]:
