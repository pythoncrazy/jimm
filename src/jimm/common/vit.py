...
class MultiHeadAttentionPoolingHead(nnx.Module):
    ...
    def __init__(
        ...
        """
        _probe_initializer = nnx.with_partitioning(nnx.initializers.zeros_init(), (None, None, "model"), mesh)
...
class VisionTransformerBase(nnx.Module):
    ...
    def __init__(
        ...
        self.patch_embeddings = nnx.Conv(
            ...
            param_dtype=param_dtype,
            rngs=rngs,
            kernel_init=nnx.with_partitioning(nnx.initializers.xavier_uniform(), (None, None, None, "model"), mesh),
            bias_init=nnx.with_partitioning(nnx.initializers.zeros_init(), ("model",), mesh),
        )
        _position_embeddings_initializer = nnx.with_partitioning(nnx.initializers.truncated_normal(stddev=0.02), (None, None, "model"), mesh)
        if self.pooling_type == "CLS":
            _cls_token_initializer = nnx.with_partitioning(nnx.initializers.zeros_init(), (None, None, "model"), mesh)
